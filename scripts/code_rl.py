"""
Reinforcement learning on code tasks (TACO, APPS, CodeContests) via "GRPO".
...
1 GPU:
python -m scripts.code_rl

8 GPUs:
torchrun --standalone --nproc_per_node=8 -m scripts.code_rl -- --run=default
"""
...
from tasks.taco import TACO
from tasks.apps_task import APPSTask
from tasks.code_contests import CodeContests

# -----------------------------------------------------------------------------
# CLI arguments
parser = argparse.ArgumentParser(description="Reinforcement learning on code tasks")
# Logging
...
# Training horizon
parser.add_argument("--num-epochs", type=int, default=1, help="number of epochs over the code task mixture")
...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(project="nanochat-code-rl", name=args.run, config=user_config)
...
train_task = TaskMixture([TACO(), APPSTask(split="train"), CodeContests(split="train")])
val_task = TaskMixture([APPSTask(split="test"), CodeContests(split="test")])
...
def get_batch():
    ...
    for example_idx in itertools.cycle(rank_indices):
        ...
        # Calculate the rewards for each sample using the task's reward() method
        rewards = []
        for sample_tokens in generated_token_sequences:
            ...
            # Calculate the reward via execution-based scoring
...
# -----------------------------------------------------------------------------
# Simple evaluation loop for code task average reward
def run_code_eval(task, tokenizer, engine,
    max_examples=None,
    num_samples=1,
    ...
    """
    Evaluates a code task and returns a list of records of evaluation outcomes.
...
for step in range(num_steps):
    ...
    if step % args.eval_every == 0:
        model.eval()
        avg_reward = torch.zeros(1, device=device)
        num_records = torch.zeros(1, dtype=torch.long, device=device)
        with autocast_ctx:
            records_iter = run_code_eval(val_task, tokenizer, engine, num_samples=1, max_examples=args.eval_examples, temperature=0.0)
            records = list(records_iter) # collect all records
        for r in records:
            avg_reward += sum(o["reward"] for o in r["outcomes"]) / len(r["outcomes"])
        num_records[0] = len(records)
        if ddp:
            dist.all_reduce(num_records, op=dist.ReduceOp.SUM)
            dist.all_reduce(avg_reward, op=dist.ReduceOp.SUM)
        avg_reward = avg_reward / num_records.item() if num_records.item() > 0 else avg_reward
        print0(f"Step {step} | Average reward: {avg_reward.item():.4f}")
        wandb_run.log({
            "step": step,
            "eval_avg_reward": avg_reward.item(),
        })
    ...
    if master_process and ((step > 0 and step % args.save_every == 0) or step == num_steps - 1):
        ...
        checkpoint_dir = os.path.join(base_dir, "coderl_checkpoints", output_dirname)
...
get_report().log(section="Code RL", data=[
