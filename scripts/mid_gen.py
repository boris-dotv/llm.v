"""
Generate text from a mid-trained model.
...
# 使用 mid train 模型
python -m scripts.mid_gen -p "Let's calculate what is x when x * 5 = 15" -s 818 -n 128

# 不指定 step 会自动找最新
python -m scripts.mid_gen -p "Let's calculate what is x when x * 5 = 15" -n 128

python -m scripts.base_gen -p "Let's calculate what is x when x * 25 = 120" -s 130500 -n 128

python -m scripts.mid_gen -p "Let's calculate what is x when x * 25 = 120" -s 818 -n 128
...
parser.add_argument('-p', '--prompt', type=str, default='What is the capital of France?')
...
# 关键：从 "mid" 加载而不是 "base"
if ddp_rank == 0:
    model, tokenizer, meta = load_model("mid", device, phase="eval", model_tag=args.model_tag, step=args.step)
