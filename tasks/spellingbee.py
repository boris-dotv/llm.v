...
1. SpellingBee: Counting number of occurrences of a letter in a word
2. SimpleSpelling: Simply spelling words

(1) is the goal, but (2) exists as a highly condensed version of part
...
in training data. Midtraining is a good place to do this.
...
# from nanochat.common import download_file_with_lock  # 不再从网络下载

# Letters of the alphabet
LETTERS = "abcdefghijklmnopqrstuvwxyz"
# Word list - 使用本地路径
WORD_LIST_PATH = "/public_hw/share/cit_ztyu/cz/nanochat/words_alpha.txt"
...
class SpellingBee(Task):
    ...
    def __init__(self, size=1000, split="train", **kwargs):
        ...
        if not os.path.exists(WORD_LIST_PATH):
            raise FileNotFoundError(f"Word list not found at {WORD_LIST_PATH}. Please download it.")
    ...
    def get_example(self, index):
        ...
        # 30% chance to lowercase template (lazy people don't use shift)
        if rng.random() < 0.3:
        ...
        # Now create of the ideal assistant response - build as parts (text + tool calls)
        assistant_parts = []
...
class SimpleSpelling(Task):
    ...
    def __init__(self, size=1000, split="train", **kwargs):
        ...
        rng.shuffle(words) # use a different word order than SpellingBee task
        self.words = words
...
if __name__ == "__main__":
    ...
    # preview SpellingBee task, first 10 examples
    task = SpellingBee()
    for i in range(10):
    ...
    # # preview SimpleSpelling task, first 10 examples
    # task = SimpleSpelling()
    # for i in range(10):
    #     ex = task.get_example(i)
    #     print("=" * 100)
